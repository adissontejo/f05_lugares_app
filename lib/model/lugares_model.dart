...
class LugaresModel with ChangeNotifier {
  ...
  UnmodifiableListView<Lugar> get lugares => UnmodifiableListView(_lugares);
  ...
  void remove(Lugar lugar) {
    _lugares.removeWhere((item) => item.id == lugar.id);
    notifyListeners();
  }

  void edit(Lugar lugar) {
    int index = _lugares.indexWhere((item) => item.id == lugar.id);
